...
- **J.1 Design-space search †** — how the final pipeline was chosen: how many configurations tried, internal validation criterion, what data it ran against: While the general pipeline was the same from the start, the exact prompt sent to the model was iteratively improved until a legible outcome was generated that conformed to all requirements for the submission.
