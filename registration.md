...
- **0.4 Pipeline diagram** — ordered steps from raw inputs to submitted file: 1. Loading raw study inputs 2. input validation/configuration 3. prompt construction 4. request generation 5. model inference 6. raw response archival 7. prediction extraction/transformation 8. creation of trial-level prediction file 9. aggregation across forecasts 10. submission file generation 11. run artifact archival.
...
## A · Scope of LLM use
- **A.1 Purpose** — every workflow stage where LLMs are used: LLMs are used to: iterative optimization of prompts; handling and processing requests; generating outcomes in the required format.
...
- **K.1 Code & materials** — link/DOI, secrets removed, determinism/seeds documented (also record the link in `metadata.json` → `code_repository` / `code_doi`): https://doi.org/10.5281/zenodo.22047422
